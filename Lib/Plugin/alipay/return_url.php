		...
		//判断该笔订单是否在商户网站中已经做过处理
			...
			if($result['status']==0){
				if($result['WIDip']==IP){
					...
					echo "<script>window.opener.parent.location.reload()</script>";
					//--反馈信息
					showmsg("充值成功!点击关闭","javascript:window.close()");
				}else{
					echo "<script>window.opener.parent.location.reload()</script>";
					showmsg("充值异常，提交和充值IP不符，充值无效，请联系管理员！","javascript:window.close()");	
				}
			}else{//说明notify已经完成了
				echo "<script>window.opener.parent.location.reload()</script>";
				showmsg("充值成功!点击关闭","javascript:window.close()");
