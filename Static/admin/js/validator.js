				   ...
				   if(data != 'success')
				   {
					  ...
					  if(mode == 1)
					  {
						  ...
						  else
						  {
							  ...
							  //jQuery('<span class="label-l"></span><span class="label-c"></span><span class="label-r"></span>').html(errmsg).insertAfter(element);
						  }
				   ...
				   else
				   {
					   ...
					   if(msgid)
						...
						else
						{
							...
							//jQuery('<span class="label-l"></span><span class="label-c"></span><span class="label-r"></span>').insertAfter(element);
						}
					   return true;
				   }
			   }
...
validator.showErr=function (element, errindex){
	...
	if(mode == 1){
			$('#msg_'+msgid+'_1').remove();$('#msg_'+msgid+'_2').remove();$('#msg_'+msgid+'_3').remove();
			msid = Math.round(Math.random()*1000000);
			element.attr('msgid',msid);
			jQuery('<span class="label label-important ml5" id="msg_'+msid+'_2">'+errmsg+'</span>').insertAfter(element);
	}
	else
	{
		alert(errmsg);
		...
		if(data != 'ajax')
		{
			if(validator.checkDatatype(obj,data)==false){
				...
				var msgid= obj.attr('msgid');
					...
					/*msid = Math.round(Math.random()*1000000);
					obj.attr('msgid',msid);
					jQuery('<span class="label-r2" id="msg_'+msid+'_1"></span><span class="label-c2" id="msg_'+msid+'_2">OK</span><span class="label-l2" id="msg_'+msid+'_3"></span>').insertAfter(obj);*/
